...
function aiTakeTurn() {
    ...
    // DEADLOCK DETECTION: track melded card count across consecutive turns per player.
    // If the melded card count hasn't increased in 3 consecutive turns, the player is stuck
    ...
    if (isStuck) {
        console.log(`[AI] ${aiPlayer.name} DEADLOCK DETECTED (${aiPlayer._stuckTurns} turns stuck) — unmelding all and replanning`);
        aiPlayer._stuckTurns = 0;

        // Unmeld everything back to hand so AI can replan from scratch
        ...
        aiPlayer._lastMeldedCount = 0;
        console.log(`[AI] ${aiPlayer.name} unmelded — hand now ${aiPlayer.hand.length} cards, replanning`);
    ...
    if (!isStuck && topDiscard && window.AIMeldPlanner.shouldTakeDiscard(topDiscard, aiPlayer.hand, wildRank)) {
        ...
        // Draw from deck — reshuffle discard into deck if empty
        let dcard = game.deck.draw();
        if (!dcard) {
            console.warn(`[AI] ${aiPlayer.name} — deck empty, reshuffling discard pile`);
            shuffleDiscardIntoDeck();
            dcard = game.deck.draw();
            console.log(`[AI] Deck size after reshuffling: ${game.deck.cards.length}`);
        }
        if (!dcard) {
            console.warn(`[AI] ${aiPlayer.name} — deck and discard both empty, skipping turn`);
            advanceTurn();
            return;
        }
        aiPlayer.hand.push(dcard);
    ...
    if (window.AIMeldPlanner.shouldAttemptMeld(aiPlayer.hand, aiPlayer.meldSets, wildRank)) {
        ...
        if (possibleMelds.length > 0) {
            ...
            if (bestMeld) {
                ...
                const meldDelay = Math.max(50, Math.round(game.timeOut / game.aiAutoPlaySpeed));
                setTimeout(() => {
                    ...
                    // PHASE 4: DISCARD
                    const discardDelay = Math.max(50, Math.round(game.timeOut / game.aiAutoPlaySpeed));
                    setTimeout(() => {
                        if (aiPlayer.hand.length > 0) {
