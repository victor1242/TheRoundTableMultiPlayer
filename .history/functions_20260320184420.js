...
function storeGameState(gameXta) {
...
function restoreGameState() { 
  ...
  // 1. Current Game State (Complex objects like Date are supported)
let gameState = {
    level: 5,
...
let state = game
// 2. Save Game: Clone and then stringify
function saveGame(state) {
    // structuredClone handles deep nesting and complex types
    const stateToSave = structuredClone(state); 
    localStorage.setItem('game_save', JSON.stringify(stateToSave));
