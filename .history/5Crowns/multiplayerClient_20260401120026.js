...
(function () {
  ...
  function renderActionButtons(g) {
    ...
    // Skip-offline-player button: visible to any connected player when current player is offline
    const currentPlayerInfo = latestState && Array.isArray(latestState.players)
      ? latestState.players.find((p) => p.id === (latestState.gameState && latestState.gameState.currentPlayerId))
      : null;
    ...
    if (skipBtn) {
      ...
      skipBtn.textContent = currentIsOffline ? ('Skip ' + currentPlayerInfo.name + '\'s turn (offline)') : '';
    }
