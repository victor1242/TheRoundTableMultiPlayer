...
(function () {
  ...
  function updateFinalTurnNotice(state) {
    ...
    (g.opponents || []).forEach((opp) => {
      if (opp && opp.isOut && opp.id) outPlayerIds.add(opp.id);
    });

    // Infer self out on this client from no cards remaining during live play.
    if (Array.isArray(g.myHand) && g.myHand.length === 0 && g.myTurnPhase === null && myPlayerId) {
      outPlayerIds.add(myPlayerId);
    }
  ...
  function announceWentOutIfNeeded(state) {
    ...
    const outIds = [];
    (g.opponents || []).forEach((opp) => {
      if (opp && opp.isOut) outIds.push(opp.id);
    });
    if (Array.isArray(g.myHand) && g.myHand.length === 0 && g.myTurnPhase === null) {
      outIds.push(myPlayerId);
    }
