...
(function () {
  ...
  // ── Config ──────────────────────────────────────────────────────────────
  // The server runs on port 3001.  When multiplayer.html is served from the
  // same origin (same host, port 3001) this works automatically.
  ...
  function nextRound() { sendAction({ type: 'nextRound' }); }
  ...
  function renderScaffoldGameState(state) {
    const gs = state && state.gameState ? state.gameState : {};
    const players = Array.isArray(state && state.players) ? state.players : [];
    const currentPlayer = players.find((p) => p.id === gs.currentPlayerId);

    setText('mp-round', 'Game Started');
    setText('mp-wild', 'Server-authoritative turns: coming in Phase 3');
    setText('mp-deck-count', 'Deck: —');
    setText('mp-current-player', currentPlayer ? ('Current: ' + currentPlayer.name) : 'Waiting for turn state…');

    const discardEl = $('mp-discard-card');
    if (discardEl) discardEl.innerHTML = '<span class="mp-card empty">—</span>';

    const handEl = $('mp-my-hand');
    if (handEl) {
      handEl.innerHTML = '<em>Connected. Full gameplay actions will appear after Phase 3 action wiring.</em>';
    }

    const meldsEl = $('mp-my-melds');
    if (meldsEl) {
      meldsEl.innerHTML = '<em>No melds yet</em>';
    }

    const oppEl = $('mp-opponents');
    if (oppEl) {
      oppEl.innerHTML = players
        .filter((p) => p.id !== myPlayerId)
        .map((p) => {
          const turnMark = p.id === gs.currentPlayerId ? ' 🔵' : '';
          return '<div class="mp-opponent"><strong>' + p.name + turnMark + '</strong><span> | Connected: ' + (p.connected ? 'Yes' : 'No') + '</span></div>';
        })
        .join('') || '<em>No opponents yet</em>';
    }

    // Hide action controls until full game payload is available.
    setVisible('mp-btn-draw-deck', false);
    setVisible('mp-btn-draw-discard', false);
    setVisible('mp-btn-meld', false);
    setVisible('mp-btn-undo-melds', false);
    setVisible('mp-btn-next-round', false);

    const meldHint = $('mp-meld-hint');
    if (meldHint) meldHint.style.display = 'none';

    hide('mp-round-over');
    showStatus('Game started. Lobby sync is working. Next step is wiring real turn actions.', false);
  }
  ...
  function renderActionButtons(g) {
    ...
    const amHost = latestState && latestState.players &&
      latestState.players[0] && latestState.players[0].id === myPlayerId;

    setVisible('mp-btn-draw-deck', isMyTurn && phase === 'draw');
    setVisible('mp-btn-draw-discard', isMyTurn && phase === 'draw' && g.discardSize > 0);
    ...
    setVisible('mp-btn-next-round', amHost && g && (g.phase === 'roundOver'));
  ...
  function wireButtons() {
    ...
    const drawDeckBtn = $('mp-btn-draw-deck');
    if (drawDeckBtn) drawDeckBtn.addEventListener('click', drawFromDeck);

    const drawDiscardBtn = $('mp-btn-draw-discard');
    if (drawDiscardBtn) drawDiscardBtn.addEventListener('click', drawFromDiscard);
    ...
    const nextRoundBtn = $('mp-btn-next-round');
