...
(function () {
  ...
  function renderActionButtons(g) {
    ...
    // Meld / undo / discard buttons
    setVisible('mp-btn-meld',       isMyTurn && phase === 'meld-discard' && selectedHandIndices.length >= 3);
    setVisible('mp-btn-discard',    isMyTurn && phase === 'meld-discard' && selectedHandIndices.length === 1);
    ...
    if (meldHint) {
      if (!isMyTurn || phase !== 'meld-discard') {
        ...
        meldHint.textContent = 'Select a card to discard it, or select 3+ cards then click Declare Meld.';
        meldHint.style.display = '';
      } else if (selectedHandIndices.length === 1) {
        meldHint.textContent = '1 card selected — click Discard Selected, or select more for a meld.';
        meldHint.style.display = '';
      } else if (selectedHandIndices.length < 3) {
        meldHint.textContent = `${selectedHandIndices.length} cards selected — select at least 3 to declare a meld.`;
        meldHint.style.display = '';
      } else {
        meldHint.textContent = `${selectedHandIndices.length} cards selected — click Declare Meld, or keep selecting.`;
        meldHint.style.display = '';
      }
    }
  }
  ...
  function wireButtons() {
    ...
    const discardBtn = $('mp-btn-discard');
    if (discardBtn) {
