...
(function () {
  ...
  function renderMyHand(g) {
    ...
    // Attach click handlers — clicks always toggle selection.
    // Use the Discard Selected button to discard a single chosen card.
  ...
  function renderActionButtons(g) {
    ...
    // Meld button is always on during meld-discard phase.
    // 1 card selected → Meld button discards.  3+ selected → Meld button declares meld.
    ...
    // Update Meld button label to reflect current selection
    const meldBtn = $('mp-btn-meld');
    if (meldBtn) {
      if (selectedHandIndices.length === 1) {
        meldBtn.textContent = 'Discard Selected';
        meldBtn.className = 'mp-btn danger';
      } else if (selectedHandIndices.length >= 3) {
        meldBtn.textContent = 'Declare Meld';
        meldBtn.className = 'mp-btn success';
      } else {
        meldBtn.textContent = 'Meld / Discard';
        meldBtn.className = 'mp-btn warning';
      }
    }

    const meldHint = $('mp-meld-hint');
    if (meldHint) {
      if (!isMyTurn || phase !== 'meld-discard') {
        meldHint.style.display = 'none';
      } else if (selectedHandIndices.length === 0) {
        meldHint.textContent = 'Select 3+ cards and click Meld / Discard to declare a meld, or select 1 card to discard it.';
        meldHint.style.display = '';
      } else if (selectedHandIndices.length === 1) {
        meldHint.textContent = '1 card selected — click Discard Selected to end your turn, or select more for a meld.';
        meldHint.style.display = '';
      } else if (selectedHandIndices.length === 2) {
        meldHint.textContent = '2 cards selected — need at least 1 more to declare a meld.';
        meldHint.style.display = '';
      } else {
        meldHint.textContent = `${selectedHandIndices.length} cards selected — click Declare Meld, or keep selecting more.`;
        meldHint.style.display = '';
      }
    }
  }
  ...
  function wireButtons() {
    ...
    if (meldBtn) {
      meldBtn.addEventListener('click', () => {
        if (selectedHandIndices.length === 1) {
          // Single card selected → discard it, ending the turn
          discardCard(selectedHandIndices[0]);
        } else if (selectedHandIndices.length >= 3) {
          // 3+ cards selected → declare meld
          declareMeld();
        } else {
          showStatus('Select 1 card to discard, or 3+ cards to declare a meld.', true);
        }
      });
    }
