...
function updatePlayerPrompt(msg) {
 ...
 console.log (argument.callee.name + ": Current player is " + game.currentPlayer.name + " (ID: " + game.currentPlayer.id + ")"                                    );
  let arg = "player" + game.players[game.currentPlayerIndex].id.substring(1) + "name";
  showMessage("Current player: " + arg + " - " + game.currentPlayer.name);
  ...
  game.currentPlayer = game.players[game.currentPlayerIndex];
   if (game.currentPlayer.hand.length+
    ...
    game.cardsDealt + 1){
     ...
     recordDraw("discard", dcardgame.players, game.currentPlayer);
    ...
    if (Array.isArray(state.players)) {
      ...
      for (let i = 0; i < state.players.length; i++) {
        ...
        if (game.players[i]) {
          ...
          if (game.players[0].hand.length === 0) dealNewRoundCards();
