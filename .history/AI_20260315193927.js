...
function aiTakeTurn() {
    ...
    if (topDiscard && window.AIMeldPlanner.shouldTakeDiscard(topDiscard, aiPlayer.hand, wildRank)) {
        ...
        if (game.discardPile.length > 0) {
            displayCard(game.discardPile[game.discardPile.length - 1], "discard", "discard");
        } else {
        ...
        if (possibleMelds.length > 0) {
            ...
            if (bestMeld) {
                ...
                setTimeout(() => {
                    ...
                    console.log(`[AI]  Meld check: currentMelded=${currentMeldedCount}, meldSize=${cardsToMeld.length}, total=${totalAfterMeld}, cardsDealt=${game.cardsDealt}, leftInHand=${cardsLeftInHand}`);
                    ...
                    if (cardsLeftInHand === 0 && totalAfterMeld < game.cardsDealt) {
                        console.warn(`[AI] Can't meld all cards without going out: melded=${totalAfterMeld}, needed=${game.cardsDealt}`);
                        // Reset and skip melding
                        resetAIMeldSelection(aiPlayer);
                    ...
                    if (!meldResult.valid) {
                        console.warn(`[AI] Meld validation failed for ${aiPlayer.name}:`, cardsToMeld.map(c => c.rank + "-" + c.suit));
                        // Reset and skip melding
                        resetAIMeldSelection(aiPlayer);
