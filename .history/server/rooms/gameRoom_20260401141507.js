...
class GameRoom {
  ...
  getActiveHostId() {
    const primaryHost = this.players[0] || null;
    if (!primaryHost) return null;
    if (primaryHost.connected) return primaryHost.id;
    const connectedFallback = this.players.find((p) => p.connected) || null;
    return connectedFallback ? connectedFallback.id : primaryHost.id;
  }
