...
function chooseDiscard(hand, wildRank) {
  ...
  hand.forEach((card, i) => {
    // Never discard wilds unless forced
    if (card.rank === 'joker' || card.rank === wRank) return;
  ...
  // Fallback: discard index 0 (should only happen if all cards are wild)
...
function playOfflineTurn(room) {
  ...
  const discardIdx = chooseDiscard(player.hand, wildRank);
