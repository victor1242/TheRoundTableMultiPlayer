...
function wildRankForRound(roundNumber) {
  // Round 1 uses 3s as wild, then 4s ... up to kings in round 11.
  const idx = Number(roundNumber) - 1;
  if (idx < 0 || idx >= RANKS.length) {
    throw new Error('Invalid round number for wild rank: ' + roundNumber);
  }
  return RANKS[idx];
}
  ...
  if (uniqueSuits.length === 1) {
    ...
    for (let i = 1; i < values.length; i++) {
      if (values[i] === values[i - 1]) {
        return { valid: false, reason: 'Run cannot contain duplicate ranks (e.g. 5,5,6)' };
      }
    ...
    return {
      valid: false,
      reason: `Run has ${gaps} missing rank(s) but only ${wilds.length} wild card(s) available`,
    };
  }

  return {
    valid: false,
    reason: 'Cards do not form a valid set or run. Runs must be same suit; sets must share rank.',
  };
}
...
function calculateRoundScore(player, wildRank, optGoingOutBonus) {
  ...
  if (player.IsOut && optGoingOutBonus) {
    score -= cardDisplayValue({ rank: wRank, value: RANK_VALUES[wRank] || 0 }, wRank);
  }
