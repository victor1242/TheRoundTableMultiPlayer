...
function recordDiscard(card, player) {
  return; // Disable discard recording for now to prevent AI overreacting to its own discards during meld phase
...
// check if meld is valid, if so move cards from hand to meldSets, if not alert user and reset meldCards and borders  
  ...
  cardsToMeld.forEach((card) => {
    ...
    if (game.currentPlayer.hand.length === 0) {
      game.currentPlayer.IsOut = true;
      isOutEl = document.getElementById(game.currentPlayer.id + "IsOut");
      if (isOutEl) {
        isOutEl.textContent = "OUT";
        isOutEl.style.color = "red";
      }
...
game.currentPlayer.hand.length
function Test() {
const isOutEl = document.getElementById( "p1IsOut");
if (isOutEl) {isOutEl.textContent = "OUT";
isOutEl.style.color = "red";  
}
}
