...
function TestB() { 
  ...
  dCard = game.discardPile[game.discardPile.length - 1];
  updateDisplay()
...
function updateDisplay() {
    ...
    if (deckCardsEl) {
      ...
      deckCardsEl.innerText = `${game.deck.length}`;
    }
    if (discardCardsEl) 
    {
...
function renderCard(card) {
  const displayElement = document.getElementById('discard-pile-display');
  if (card) {
...
function displayNextCard() {
  ...
  if (discardPile.length === 0) {
    renderCard(null); // Blank out the display
  ...
  console.log("Retrieved card:", topCard);
  ...
  const nextTopCard = discardPile.length > 0 ? discardPile[discardPile.length - 1] : null;
  renderCard(nextTopCard);
}

// Initial display of the top card when the page loads
document.addEventListener('DOMContentLoaded', (event) => {
  const initialTopCard = discardPile.length > 0 ? discardPile[discardPile.length - 1] : null;
  renderCard(initialTopCard);
});
