...
function draw() {
 ...
 // else showMessage ("call to game.deck.draw() corrupted")  
    ...
    while (!card) {
      ...
      console.log("Deck size after shuffling discard in:", game.deck.length);
      setPlayerDrawMode()
    }
...
// check if meld is valid, if so move cards from hand to meldSets, if not alert user and reset meldCards and borders  
  ...
  cardsToMeld.forEach((card) => {
    card.styleBorder = game.meldingColour;
  });
      ...
      if (possibleMelds.length > 0) {
          ...
          if (bestMeld) {
              ...
              bestMeld.indices.forEach(idx => {
                  ...
                  if (cardEl) {
                      cardEl.style.border = game.aiManualStepMode ? "3px solid orange" : getMeldGroupColour(game.currentPlayer.meldGroup);  
...
function validateMeld(group, options = {}) {
 ...
 function advanceTurn(options = {}) {
  ...
  if (game.currentPlayer.aiPlayer && game.aiAutoPlayEnabled && !game.aiAutoPlayPaused) {
    ...
    console.log(`[advanceTurn] NOT scheduling AI turn: aiPlayer=${game.currentPlayer.aiPlayer}, autoPlayEnabled=${game.aiAutoPlayEnabled}, paused=${game.aiAutoPlayPaused}`);
  }
 }
...
function displayNextDiscard() {
  ...
  if (game.discardPile.length === 0) {
    ...
    console.log("Discard pile is now empty.");
    return;
  }
