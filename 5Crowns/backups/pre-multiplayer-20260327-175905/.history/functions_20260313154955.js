...
function updatePlayerReference() {
  ...
  if(Player.aiPlayer) {
    const aiEl = document.getElementById(Player.id + "ai-h");
    const myButton = document.getElementById("myButton");

// Create a new image element
const buttonImage = document.createElement("img");

// Set the image source (replace 'path/to/your/image.png' with your image path)
buttonImage.src = "path/to/your/image.png";

// Set an alt attribute for accessibility
buttonImage.alt = "Description of the button's action";

// Optional: set image dimensions
buttonImage.style.width = "30px";
buttonImage.style.height = "30px";

// Append the image element to the button
myButton.appendChild(buttonImage);

    if (aiEl) aiEl.textContent = "./android.png";
    aiEl.style.backgroundColor = "";
  }
  ...
  // Create round object
   roundObj = {
    ...
    players: game.players.map((p) => ({
      name: p.name,
      IsOut: p.IsOut,
      cards: p.hand.length + p.meldCards.length,
      wentOutScore: p.wentOutScore ?? 0,
      roundScore: p.roundScore ?? 0,  
      gameScore: p.gameScore ?? 0,
      hand: p.hand.length,
      melds: p.meldSets.length,
    })),
  };
      ...
      if (possibleMelds.length > 0) {
          ...
          if (bestMeld) {
              ...
              bestMeld.indices.forEach(idx => {
                  ...
                  if (cardEl) {
                      cardEl.style.border = "6px solid yellow"; 
