...
//=======================================================
let deck = Array.from({length: 52}, (_, i) => i + 1); // 52 cards
let discardPile = [];

function updateUI() {

    const deckEl document.getElementById('deck-count').innerText = game.deck.cards.length;
    document.getElementById('discard-count').innerText = game.discardPile.length;

    // Toggle deck visual if empty
    document.getElementById('deck').style.visibility = game.deck.cards.length > 0 ? 'visible' : 'hidden';
}

// Example action: draw a card
function drawCard() {
...
// Initialize
updateUI();
// Add click listener to deck to draw
