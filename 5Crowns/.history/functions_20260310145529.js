...
function  drawDiscardHint() {
  ...
  const discardValue = game.discardPile[0].value;
  const handValues = hand.map((c) => c.value);
  const hasWild = hand.some((c) => c.wild);
  if (shouldTakeDiscard(game.discardPile[0], game.currentPlayer.hand, game.roundNumber + 2)) {
