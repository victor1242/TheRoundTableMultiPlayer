...
// hand click
    ...
    if (!game.currentPlayer || !cardEl.classList.contains(game. currentPlayer.id + "card")) {
      return;
    } 
    // procees hand card click - either melding or discarding depending on current mode
    prcessPlayerHandCardClick(cardEl);
  }

  );
    if (game.currentPlayer.melding) { 
      // Melding - if hand card clicked, toggle meld selection 
      // on - border colour
      // off - no border colour
      ...
      // Ensure there are no cards currently selected for melding - if there are, reject discard until player confirms they want to unselect melds
      if (cardEl.style.border !== "")  return;
      else {
        // discarding selectecd card from hand to discard pile
        ...
        // check if trying to discard wild card before final turn phase, if option is selected, warn player that he is discarding a wild card which the next player will probaly appreciate, and ask for confirmation
        const wildRank = String(game.roundNumber + 2);
        const isWild =
          selectedCard?.rank === "joker" || selectedCard?.rank === wildRank;
        if (isWild) {
          if (!(confirm("Are you sure that you want to discard a wild card? "))) {
            return;
          }
        }
        const meldedCount = getMeldedCardCount(game.currentPlayer);
        const totalCards = 3 * meldedCount + game.currentPlayer.hand.length;
...
} // <-- Add this closing brace to match the extra opening brace above
