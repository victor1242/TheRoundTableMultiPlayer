...
function applyMeldingStrategy( ) {
  ...
  if (window.AIMeldPlanner.shouldUnmeldAndRemeld(game.currentPlayer.hand, game.currentPlayer.meldSets, arguments, (game.roundNumber + 2))) {
  ...
  // PHASE 3: MELD
  if (window.AIMeldPlanner.shouldAttemptMeld(game.currentPlayer.hand, game.currentPlayer.meldSets, game.round)) {
      ...
      if (possibleMelds.length > 0) {
          ...
          if (bestMeld) {
              ...
              bestMeld.indices.forEach(idx => {
                  ...
                  if (cardEl) {
                      cardEl.style.border = getMeldGroupColour(game.currentPlayer.meldGroup);
