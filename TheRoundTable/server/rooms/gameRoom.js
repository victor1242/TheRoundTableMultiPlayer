...
class GameRoom {
  ...
  toClientState(forPlayerId) {
    ...
    const playerConnected = Object.fromEntries(this.players.map((p) => [p.id, p.connected]));
    gameState.opponents = gameState.opponents.map((opp) =>
      Object.assign({}, opp, { name: playerNames[opp.id] || opp.id, connected: playerConnected[opp.id] ?? true })
    );
