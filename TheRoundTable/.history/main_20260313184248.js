...
displayCard(game.discardPile[0], "discard");
...
function updateCurrentPlayerReference() {
  ...
  game.players.forEach((p) => {
    ...
    if (aiBadgeEl) {
      if (p.aiPlayer) {
        aiBadgeEl.innerHTML = '<img class="ai-icon" src="./android.png" alt="AI player" />';
...
function displayCard(card, cardId, mode) {
  ...
  cardImg.src = imgsrc;
  if (mode === "meld") cardImg.style.border = "6px solid yellow";
  document.getElementById(cardId).innerText = "";
  document.getElementById(cardId).append(cardImg);
  showMessage(card.rank + ": " + card.suit + " " +  cardId);
}
   ...
   if (game.currentPlayer.hand.length+
    ...
    game.cardsDealt + 1){
     ...
     if (game.discardPile.length === 0)  document.getElementById("discard").innerHTML = "";
      ...
      if (game.discardPile.length === 0) document.getElementById  ("discard").innerHTML = "";
      else displayCard(game.discardPile[game.discardPile.length - 1], "discard");
      }
