...
function aiTakeTurn() {
    ...
    if (!isStuck && topDiscard && window.AIMeldPlanner.shouldTakeDiscard(topDiscard, aiPlayer.hand, wildRank)) {
        ...
        if (game.discardPile.length > 0) {
            displayCard(game.discardPile[game.discardPile.length - 1], "discard-count", "discard-count");
        } else {
        ...
        if (!dcard) {
            ...
            // In a real game, we would reshuffle the discard pile into the deck here
            // For now, just skip drawing and proceed to meld/discard phase
            } 
        else {
