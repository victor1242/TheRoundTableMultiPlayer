...
let playerWhoWentOut = -1;  // Track which player went out first
let finalTurnPlayers = new Set();  // Track who has had their final turn

// Global reference to the active player, updated each turn
...
function updatePlayerPrompt(msg) {
  ...
  if (!promptEl) promptEl =  document.innerText=msg
}
...
function recordDiscard(card, player) {
  if (!game.recentDiscards) game.recentDiscards = [];
  game.recentDiscards.push({
    rank: card?.rank,
    suit: card?.suit,
    playerId: player?.id,
  });
  if (game.recentDiscards.length > 6) game.recentDiscards.shift();
}getValueRank
...
function renderPlayerHand(Player) {
    ...
    for (let meld of Player.meldSets) {
      if (Array.isArray(meld) && meld.length > 0) {
        for (let card of meld) {
          ...
          if (el && card) displayCard(card, el.id);
          cardIdx++;
        }
      }
    }
  ...
  for (let k = 0; k < Player.hand.length; k++) {
    ...
    if (el && Player.hand[k]) displayCard(Player.hand[k], el.id);
    cardIdx++;
  }
}
...
function advanceTurn(options = {}) {
  ...
  game.AIPlayers=0
  game.players.forEach((p) => {if(p.isAI) game.AIPlayers++});

  if (aiManualStepMode && !forceAdvance) {
    aiPendingAdvance = true;
    updateAIStepButton();
    return;
  }
  aiPendingAdvance = false;
  if (game.currentPlayerIndex !== -1) {
    // Mark current player as having had their final turn (if in final phase and NOT out)
    if (playerWhoWentOut !== -1 && !game.currentPlayer.IsOut) {
      finalTurnPlayers.add(game.currentPlayerIndex);
      console.log(`[Final Turn] ${game.currentPlayer.name} has played. Total final turns: ${finalTurnPlayers.size}`);
    }

    if (playerWhoWentOut !== -1) {
      const playersNotOut = game.players.filter((p) => !p.IsOut).length;
      if (finalTurnPlayers.size >= playersNotOut) {
        console.log(`[Final Turns Complete] All ${playersNotOut} remaining players have had their final turn. Ending round.`);
        saveScoreBoard();
        saveGameState();
        playerWhoWentOut = -1;
        finalTurnPlayers = new Set();
        NextRound();
        return;
      }
    }
    ...
    // Advance to next player, skipping any who are out
    let nextIndex = (game.currentPlayerIndex + 1) % game.players.length;
    let attemptsCount = 0;
    
    // Skip players who are out
    while (game.players[nextIndex].IsOut && attemptsCount < game.players.length) {
    ...
    game.currentPlayer.canDiscard = false;
    /* turn meld on
  ...
  // Check if all players are out (round should end)
  const anyoneNotOut = game.players.some(p => !p.IsOut);
  if (!anyoneNotOut) {
    // All players are out, end the round
    console.log("[Round End] All players have gone out.");
    saveScoreBoard();
    saveGameState();
    playerWhoWentOut = -1;
    finalTurnPlayers = new Set();
...
function addHandCardListeners() {
  if (handCardListenersBound) return;
  handCardListenersBound = true;
  console.log("Adding HAND CARD listeners...");

  document.addEventListener("click", function handleClick(event) {
    // Delegate card clicks to avoid re-binding on re-render.
...
// hand click
  ...
  //  let relative = game.currentPlayerIndex + 1;
 //   if (cardClass !== "p" + relative + "card") return;

    if (game.currentPlayer.melding) { 
      if (cardEl.style.border === "") {
        cardEl.style.border = getMeldGroupColour(
          game.currentPlayer.meldGroup,
        );
      ...
      if (cardEl.style.border !== "") {
        ...
        if (!selectedCard) {
          console.warn(
            "[Discard] Selected card not found for hand index",
            {
              player: game.currentPlayer?.name,
              handIndex,
              cardIndexInt,
              meldedCount: getMeldedCardCount(game.currentPlayer),
              handSize: game.currentPlayer.hand.length,
            },
          );
          return;
        }
        const wildRank = String(game.roundNumber + 2);
        const hasNonWild = game.currentPlayer.hand.some(
          (card) => card.rank !== "joker" && card.rank !== wildRank,
        );
        const isLastCard = game.currentPlayer.hand.length === 1;
        const isWild =
          selectedCard?.rank === "joker" || selectedCard?.rank === wildRank;
        if (isWild && playerWhoWentOut === -1 && hasNonWild && !isLastCard) {
        ...
        const totalCards = meldedCount + game.currentPlayer.hand.length;
        if (game.currentPlayer.hand.length === 1 && totalCards < game.cardsDealt) {
          if (!game.currentPlayer.isAI) {
            alert("You must meld all cards before discarding your last card.");
          }
          console.log(`[Discard Check] ${game.currentPlayer.name} cannot go out: melded=${meldedCount}, hand=${game.currentPlayer.hand.length}, total=${totalCards}, required=${game.cardsDealt}`);
          return;
        }
        ...
        recordDiscard(xCard, game.currentPlayer);
        ...
        if (game.currentPlayer.hand.length === 0) {
          ...
          if (playerWhoWentOut === -1) {
            playerWhoWentOut = game.currentPlayerIndex;
            finalTurnPlayers = new Set();
            console.log(`[Going Out] ${game.currentPlayer.name} (index ${playerWhoWentOut}) went out. Starting final turn phase.`);
          }
          if (!game.currentPlayer.isAI) {
...
function discardFromHand(player, handIndex, options = {}) {
  ...
  if (player.hand.length === 0) {
    ...
    if (playerWhoWentOut === -1) {
      playerWhoWentOut = game.currentPlayerIndex;
      finalTurnPlayers = new Set();
      console.log(`[Going Out] ${player.name} (index ${playerWhoWentOut}) went out. Starting final turn phase.`);
    }
    if (!player.isAI) {
...
function performAITurn() {
  ...
  if (!aiPlayer.canDraw && !aiPlayer.canDiscard) {
    aiPlayer.canDraw = true;
    UpdatecurrentPlayerReference();
  }
  ...
  if (aiPlayer.canDiscard && !aiPlayer.melding) {
    ...
    if (debugLog) {
      console.log(`[MELD-PHASE] ${aiPlayer.name}: canDiscard=${aiPlayer.canDiscard}, melting=${aiPlayer.melding}, turnKey=${turnKey}, hasPlan=${hasPlan}`);
    }
...
function Meld() {
  ...
  if (game.currentPlayer.meldCount === 0) {
    if (game.currentPlayer.melding) {
      ...
      game.currentPlayer.meldGroup -= 1;
 //     if (game.currentPlayer.hand.length < 3) changeMeldColor("0");
      return;
    } else {
      game.currentPlayer.melding = true;
      game.currentPlayer.meldGroup += 1;
...
// check if meld is valid, if so move cards from hand to meldSets, if not alert user and reset meldCards and borders  
  ...
  if (!meldResult.valid) {
    ...
    alert("Invalid Meld");
    // Reset meld mode and clear selections so player can try again.
  ...
  cardsToMeld.forEach((card) => {
    ...
    if (game.currentPlayer.hand.length === 0) {
      game.currentPlayer.IsOut = true;
      // When someone goes out, track it for final turn phase
      if (playerWhoWentOut === -1) {
        playerWhoWentOut = game.currentPlayerIndex;
        finalTurnPlayers = new Set();
        console.log(`[Going Out] ${game.currentPlayer.name} (index ${playerWhoWentOut}) went out. Starting final turn phase.`);
      }
      // SHOW GOING MESSAGE IF ANY PLAYERSBARE NOT ai
...
function updateRoundUI() {
  ...
  if (roundNumberEl) {
    roundNumberEl.textContent = "Round: " + String(game.roundNumber);
  }
  showRoundAndWilds();
}
...
function draw() {
  ...
  if (game.currentPlayer.canDraw) {
    ...
    game.currentPlayer.canDraw = false;
    game.currentPlayer.canDiscard = true;
    renderPlayerHand(game.currentPlayer);
    //   displayCard(card, ActivePlayer.id + "card" + ActivePlayer.hand.length);
  } else {
    if (game.currentPlayer.meldCount === game.roundNumber + 2) {
      alert(
        game.currentPlayer.name + " you have melded all your cards and gone out!",
      );
    } else {
      alert(game.currentPlayer.name + " you have already drawn a card this turn");
    }
  }
}
...
function refillDeckFromDiscard() {
  ...
  game.deck.shuffle();
  displayCard(topDiscard, "discard");
...
function addEventListeners() {
  ...
  const bindHandlers = () => {
    ...
    if (NewGameEl) NewGameEl.addEventListener("click", NewGame);
...
function UpdatecurrentPlayerReference() {
  ...
  for (let p of game.players) {
    ...
    const el3 = document.getElementById(p.id +"canDraw");
    if (el3) el3.textContent = "canDraw: " + p.canDraw;

    const el4 = document.getElementById(p.id +"canDiscard");
    if (el4) el4.textContent = "canDiscard: " + p.canDiscard;
...
function NextRound() {
 ...
 // window.location.href = "ScoreBoard.html";
  ...
  // Reset final turn tracking for new round
  playerWhoWentOut = -1;
  finalTurnPlayers = new Set();
  ...
  if (game.deck.cards.length < 116) {
    game.deck = new Deck();
    game.deck.shuffle();
    console.log(`[DECK] New deck created with ${game.deck.cards.length} cards`);
  ...
  if (aiAutoPlayEnabled && !aiAutoPlayPaused && game.currentPlayer && game.currentPlayer.isAI && game.roundNumber <= 11) {
    scheduleAITurn(800);
  }
}
    ...
    if (Array.isArray(state.players)) {
      ...
      for (let i = 0; i < state.players.length; i++) {
        ...
        if (game.players[i]) {
          ...
          game.players[i].canDraw = state.players[i].canDraw;
          game.players[i].canDiscard = state.players[i].canDiscard;
...
//Listeners();
  ...
  // Re-render UI after restoring state
...
if (!restoredState) {
  ...
  game.newRound = true;
  // Fallback: ensure logInActivePlayers is available globally
 
  logInPlayers(); // Removed direct call to prevent ReferenceError
  addHandCardListeners(); 
  ...
  if (game.newRound) {
   ...
   //   displayCardBack("deck");
      ...
      if (game.currentPlayerIndex !== -1 && game.players[game.currentPlayerIndex]) {
        ...
        if (game.currentPlayer.isAI && aiAutoPlayEnabled && !aiAutoPlayPaused) {
          scheduleAITurn(1000);
        }
      } else {
        console.error(
