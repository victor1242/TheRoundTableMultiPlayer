...
function addEventListeners() {
  ...
  const bindHandlers = () => {
    ...
    const aiStepEl = document.getElementById("ai-Step");
    if (aiStepEl) aiStepEl.addEventListener("click", aiStep);

    const drawDiscardHintEl = document.getElementById("draw-discard-hint");
    if (drawDiscardHintEl) drawDiscardHintEl.addEventListener("click", drawDiscardHint);
...
function aiStep() {
 const aiBrakeEl = document.getElementById("ai-Step")
 aiBrakeEl.style.backgroundColor = "";
}
...
function aiWaitUserOk()
  { return
    ...
    const aiStepEl = document.getElementById("ai-Step");
    aiStepEl.style.backgroundColor = "";
  }
  ...
  // Create round object
   roundObj = {
    ...
    players: game.players.map((p) => ({
      name: p.name,
      IsOut: p.IsOut,
      cards: (Array.isArray(p.hand) ? p.hand.length : 0) + (Array.isArray(p.meldCards) ? p.meldCards.length : 0),
...
function applyMeldingStrategy(aiPlayer, wildRank) {
gStrategy(aiPlayer, wildRank) {

    // PHASE 2: UNMELD & REMELD (if beneficial)
  if (window.AIMeldPlanner.shouldUnmeldAndRemeld(Player.hand, Player.meldSets, wildRank)) {
      console.log(`${Player.name} unmelding to optimize melds`);
      
      // Unmeld all cards back to hand
      for (let i = 0; i < Player.meldSets.length; i++) {
          for (let k = 0; k < Player.meldSets[i].length; k++) {
              Player.meldSets[i][k].bckgrndColour = "";
              Player.meldSets[i][k].styleBorder = "";
              Player.hand.push(Player.meldSets[i][k]);
          }
      }
      Player.meldSets = [];
      Player.meldCards = [];
  ...
  // PHASE 3: MELD
  if (window.AIMeldPlanner.shouldAttemptMeld(Player.hand, Player.meldSets, wildRank)) {
      const possibleMelds = window.AIMeldPlanner.findPossibleMelds(Player.hand, wildRank);
      console.log(`[AI] ${Player.name} found ${possibleMelds.length} possible melds`);
      
      if (possibleMelds.length > 0) {
          const bestMeld = window.AIMeldPlanner.selectBestMeld(possibleMelds, wildRank);
          
          if (bestMeld) {
              console.log(`[AI] ${Player.name} melding ${bestMeld.size} cards (${bestMeld.type})`);
              ...
              bestMeld.indices.forEach(idx => {
                  ...
                  if (cardEl) {
                      cardEl.style.border = getMeldGroupColour(Player.meldGroup);
                      Player.meldCount += 1;
                  }
              });
            } 
...
function validateMeld(group) {
 ...
 function advanceTurn(options = {}) {
  ...
  if (game.currentPlayer.aiPlayer) {
     aiTakeTurn()
// if next player is AI, trigger their turn logic
    let nextPlayer  = (game.currentPlayerIndex + 1) % game.players.length;
    if (game.players[nextPlayer].aiPlayer) setTimeout(advanceTurn, 0);
    }                          
...
function xvalidateMeld(group, options = {}) {
