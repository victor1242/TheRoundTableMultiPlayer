...
//=======================================================
// Sample data representing your card deck
const deck = [
  { id: 1, name: 'Card 1', count: 1, imageUrl: 'placeholder1.jpg' },
  { id: 2, name: 'Card 2', count: 1, imageUrl: 'placeholder2.jpg' },
  // ... more cards
];
...
function renderDeck() {
  ...
  cardDeckElement.innerHTML = ''; // Clear previous display
  let totalCards = 0;
  ...
  deck.forEach(cardData => {
    ...
    cardCount.textContent = `Count: ${cardData.count}`;
  ...
  cardsRemainingElement.textContent = totalCards;
}

// Initial render
renderDeck();

// Example function to update the deck and re-render (e.g., when a card is removed)
function removeCard(cardId) {
  const cardIndex = deck.findIndex(card => card.id === cardId);
  if (cardIndex > -1) {
    deck[cardIndex].count--;
    if (deck[cardIndex].count === 0) {
      deck.splice(cardIndex, 1); // Remove card entirely if count is zero
    }
    renderDeck(); // Re-render the display
  }
}
