...
function renderPlayerHand(Player) {
  // Clear previous hand display and borders
  for (let k = 1; k <= 12; k++) {
    const el = document.getElementById(Player.id + "card" + k);
    if (el) {
      el.innerHTML = "";
      // Clear border unless this will be a melded card
      el.style.border = "";
    }
  }
  game.meldCount = 0

  // Render meldSets (2D array) with borders
  let cardIdx = 0;
  let meldGroupNum = 1;
  if (Array.isArray(Player.meldSets)) {
    for (let meld of Player.meldSets) {
      if (Array.isArray(meld) && meld.length > 0) {
        game.meldCount = meld.length
        ...
        for (let card of meld) {
          ...
          if (el && card) {
            ...
            // Apply meld border to melded cards
            el.style.border = meldBorder;
          }
          cardIdx++;
        }
        meldGroupNum++;
  ...
  // Render remaining hand cards after melds (no borders unless actively melding)
  for (let k = 0; k < Player.hand.length; k++) {
    const el = document.getElementById(Player.id + "card" + (cardIdx + 1));
    if (el && Player.hand[k]) {
      // Clear styleBorder on hand cards to prevent old meld borders from showing
      Player.hand[k].styleBorder = "";
      displayCard(Player.hand[k], el.id);
      // Hand cards don't get borders from renderPlayerHand
      // Borders are added by user clicking or AI marking
    }
    cardIdx++;
  }
}
...
function renderGamesDirectory() {
  ...
  console.log("Rendering games directory...", listEl);
  if (!listEl) return;
...
function addHandCardListeners() {
  document.addEventListener("click", function handleClick(event) {
    const cardEl = event.
    target.closest(".p1card, .p2card, .p3card");
    if (!cardEl) return;
// hand click
...
function storeGameState(gameState) {
  const newStorageKey = getCurrentGameStorageKey();
  const savedData = structuredClone(gameState);
  localStorage.setItem(newStorageKey, JSON.stringify(savedData));  
  console.log("Game state stored in localStorage under key:", newStorageKey);
  upsertGamesDirectoryEntry();
}

function reLoadGameState(gameIdOverride) {
  // get the key for the last game from localStorage and load the game state from that key, if it exists
  const storageKey = gameIdOverride
    ? getGameStorageKey(gameIdOverride)
    : getCurrentGameStorageKey();
