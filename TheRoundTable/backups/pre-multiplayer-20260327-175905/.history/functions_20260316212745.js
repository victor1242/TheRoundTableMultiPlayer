...
function updateUI() {
    ...
    const deckCEl = document.getElementById('deck-count');
    if (deckEl)
    deckEl.innerText = game.deck.cards.length;
    const discardEl = document.getElementById('discard-count');
    if (discardEl)
    discardEl.innerText = game.discardPile.length;

    // Toggle deck visual if empty
   const deckEl = document.getElementById('deck');
   if (deckEl) deckEl.style.visibility = game.deck.cards.length > 0 ? 'visible' : 'hidden';
}
