...
function xxsaveGameState() {
  ...
  ////console.log(stateToSave.players[0].hand);
...
function restoreGameState(storageKey) {
  ...
  const resolvedKey = console.log("Saving game state to localStorage...");
  ////console.log(stateToSave.players[0].hand);
  const storageKey = getCurrentGameStorageKey(true);
  localStorage.setItem(storageKey, JSON.stringify(stateToSave));
  upsertGamesDirectoryEntry();storageKey || getCurrentGameStorageKey();
