...
updateDisplay()
displayCard(game.discardPile[0], "discard-count");
...
if (gameSelectedOption === "Restore Game") restoreGameState();
...
function addHandCardListeners() {
  document.addEventListener("click", function handleClick(event) {
    alert("Hand Card Listener Clicked");
...
function discardCardSelected(cardEl) {
      ...
      if (isWild) {
        ...
        updateDisplay()
        displayCard(xCard, "discard-count", "discard" );
  ...
  game.currentPlayer = game.players[game.currentPlayerIndex];
   if (game.currentPlayer.hand.length+
    ...
    game.cardsDealt + 1){
     ...
     if (game.discardPile.length === 0)  document.getElementById("discard-count").innerHTML = "";
     else 
      {
      ...
      if (game.discardPile.length === 0) document.getElementById  ("discard-count").innerHTML = "";
      else displayCard(game.discardPile[game.discardPile.length - 1], "discard-count");
      }
