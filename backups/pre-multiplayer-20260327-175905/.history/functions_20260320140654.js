...
// 1. Check if localStorage is supported by the browser
if (typeof(Storage) !== "undefined") {
    // localStorage is supported, proceed with a try-catch block for safer usage
    try {
        const keyName = 'username';
        const valueToSave = 'Alice';

        // Save data to local storage
        localStorage.setItem(keyName, valueToSave);
        console.log(`Saved: ${keyName} = ${localStorage.getItem(keyName)}`);

        // To store complex data types like objects or arrays, use JSON.stringify()
        const userPreferences = { theme: 'dark', fontSize: 14 };
        localStorage.setItem('preferences', JSON.stringify(userPreferences));
        console.log("Saved preferences object as string.");

    } catch (e) {
        if (e.name === "QuotaExceededError") {
            alert("Error: Local storage is full.");
        } else {
            console.error("An unexpected error occurred while saving to local storage:", e);
        }
    }
} else {
  ...
  let x=0
    /*
  while(game.discardPile.length < 50) {
