...
function Test() { 
const deckEl = document.getElementById("discard-count");

deckEl.style.color = "white";
if (deckEl) deckEl.addEventListener("click", discard); 

    /* draw()
   
  const drawnCard = game.deck.draw();
 
  displayCard(drawnCard, "discard-count", "discard"   );
  updateDisplay()
*/
...
function draw() {
  ...
  if (!card) {
    ...
    take discard pile - 1 card and shuffle into deck, then draw again
...
function shuffleDiscardIntoDeck() {
    ...
    // Use the spread syntax or concat() to merge the arrays
    deck.push(...discardPile-1); 

    // 2. Clear the original discard pile array
    discardPile.length = 0; // or discardPile = [];

    // 3. Shuffle the entire deck using the Fisher-Yates algorithm
    shuffleDeck(deck); 

    console.log("Discard pile shuffled into the deck. New deck size:", deck.length);
}